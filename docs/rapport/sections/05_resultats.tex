...
Le protocole détaillé (nearest centroid, $k^*$ sur val, bootstrap) est dans
\texttt{docs/evaluation\_protocol.md}.
...
\caption{Matrice de confusion cluster prédit vs.\ cluster réel (épisodes anomalie avec
  alerte avant injection, seuil 0.3). Les confusions hors diagonale quantifient les
  erreurs de typage en conditions d'alerte réelle.}
\label{fig:confusion_clusters}
\end{figure}
...
\paragraph{Mise en garde --- circularité d'évaluation.}\footnote{L'AUROC $= 0.973$
reporté ici mesure la prédiction des labels cluster produits par EWAT lui-même
depuis l'embedding STGCN. C'est une évaluation \emph{auto-référente} : le pipeline
retrouve son propre partitionnement. Sur cible Chaos Mesh indépendante (15 scénarios,
vérité terrain), le pipeline atteint macro-AUROC $= 0.920$ [0.878, 0.956]
sur ewat\_v4\_strat (LR-OvR sur features brutes flatten, sans STGCN) ---
voir section~\ref{sec:robustness} (Stress tests et architecture v2) et
section~\ref{sec:scenario_baseline} (B3/B4 baselines).}
Le chiffre $0.973$ doit être interprété comme mesure de \emph{cohérence interne}
du clustering EWAT, pas comme métrique prédictive indépendante. La section~\ref{sec:robustness}
détaille les stress tests A1--A5 qui établissent ce diagnostic et l'architecture v2
qui fournit la métrique défensive.

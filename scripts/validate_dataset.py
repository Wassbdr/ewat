...
def _load_artifacts(
    run_dir: Path,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, pd.DataFrame, pd.DataFrame, dict]:
    ...
    return signal, signal_mask, adjacency, labels, graph_stats, metadata
...
def run_checks(
    ...
    signal, signal_mask, adjacency, labels, graph_stats, metadata = _load_artifacts(run_dir)

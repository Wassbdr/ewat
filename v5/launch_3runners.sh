...
run_cmd() {
  echo "cd '$V5' && export PYTHONPATH='$SRC' V5_KUBE_CONTEXT='$KCTX' V5_NODE_IP='$NODE_IP' V5_PROM_NODEPORT='$PROM_NP' V5_LOKI_NODEPORT='$LOKI_NP' && python -m collect.run_campaign --namespace $1 --address http://$NODE_IP:$2 --rep-start $3 --rep-end $4 --reps $REPS --pf-offset $5 --out-root '$OUT' --users $USERS --reset-every $RESET --held-out-cap $HELDCAP --ram-ceiling $RAMCEIL 2>&1 | tee -a '$OUT/_campaign_$1.log'"
}
